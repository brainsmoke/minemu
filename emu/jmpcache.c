...
void add_jmp_mapping(char *addr, char *jit_addr)
{
	...
	for (i=hash; i<JMP_CACHE_SIZE; i++)
		if ( jmp_cache.addr[i] == NULL )
		{
			jmp_cache.addr[i] = addr;
			jmp_cache.jit_addr[i] = jit_addr;
			return;
		}
	...
	jmp_cache.addr[hash] = addr;
	jmp_cache.jit_addr[hash] = jit_addr;
...
static void jmp_cache_clear(char *addr, unsigned long len)
{
	...
	{
		if ( contains(addr, len, jmp_cache.addr[i]) )
			jmp_cache.addr[i] = jmp_cache.jit_addr[i] = NULL;
	...
	for (i=0; i<JMP_CACHE_SIZE; i++)
	{
		if ( jmp_cache.addr[i] == NULL )
			last = i;
		else if ( HASH_OFFSET(last, jmp_cache.addr[i]) <
		          HASH_OFFSET(i, jmp_cache.addr[i]) )
		{
			tmp_addr = jmp_cache.addr[i];
			tmp_jit_addr = jmp_cache.jit_addr[i];
			jmp_cache.addr[i] = jmp_cache.jit_addr[i] = NULL;
...
char *find_jmp_mapping(char *addr)
{
	...
	for (i=hash; i<JMP_CACHE_SIZE; i++)
		if ( (jmp_cache.addr[i] == addr) || (jmp_cache.addr[i] == NULL) )
			return jmp_cache.jit_addr[i];
...
void move_jmp_mappings(char *jit_addr, unsigned long jit_len, char *new_addr)
{
	...
	for (i=0; i<JMP_CACHE_SIZE; i++)
		if ( !contains(jit_addr, jit_len, jmp_cache.jit_addr[i]) )
			jmp_cache.jit_addr[i] += diff;
}
*/
